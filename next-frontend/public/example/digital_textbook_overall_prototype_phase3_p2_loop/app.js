(function () {
  ...
  function setView(view) {
    ...
    if (view === "graph" && state.graphId.startsWith("CG-")) {
      state.graphTaskId = "P2-T3";
      state.graphId = "P2T3-N03";
    }
  ...
  function selectedGraphNode() {
    return allGraphNodes().find((node) => node.id === state.graphId) || normalizeGraphNode(data.graph.courseChain[1]);
  }
  ...
  function setGraphTask(taskId) {
    ...
    if (task && !task.nodeIds.includes(state.graphId)) {
      state.graphId = task.nodeIds.includes("P2T3-N03") ? "P2T3-N03" : task.nodeIds[0];
    }
    render();
  }
  ...
  function renderContextPanel() {
    ...
    contextPanel.innerHTML = `
      ...
      <div class="panel-stack">
        ...
        <div class="panel-item">
          <strong>下一步产物</strong>
          <span>samples/digital_textbook_overall_prototype_phase3_p2_loop/ 完成后进入浏览器QA、内部全量评审和治理更新。</span>
        </div>
        <div class="panel-item">
          <strong>外部门禁</strong>
    ...
    workspace.innerHTML = `
      ...
      <section class="section">
        ...
        <div class="project-grid">
          ${data.projects.map((project) => `
            <article class="project-card" data-current="${project.id === "P2"}">
              <div class="badge-row">
                <span class="badge">${escapeHtml(project.id)}</span>
                <span class="badge ${project.id === "P2" ? "deep" : ""}">${escapeHtml(project.status)}</span>
              </div>
      ...
      <section class="section">
        ...
        <div class="flow-grid">
          ...
          <article class="flow-card"><h3>项目二</h3><p>展示测试采集主线、异常处理分支、测试数据分析输出三项任务的关系。</p></article>
          <article class="flow-card"><h3>P2-T3候选</h3><p>先验证P2-T3是否适合作为下一个任务级深样章，不提前宣称已经完成。</p></article>
          <article class="flow-card"><h3>状态回写</h3><p>测试任务产出回到课程能力图谱，形成能力节点、资源和评价的闭环。</p></article>
        </div>
      </section>
    `;
  ...
  function renderPhase2Loop() {
    const loop = data.phase2Loop;
    if (!loop) return "";
    ...
    return `
      <section class="section">
        ...
        <div class="loop-route">
          ${loop.route.map((item, index) => `
            <article class="route-card">
              <div class="badge-row">
                <span class="badge deep">${escapeHtml(item.id)}</span>
                <span class="badge">${escapeHtml(item.role || (index === 0 ? "入口" : index === 1 ? "中枢" : "出口"))}</span>
              </div>
      ...
      <section class="section">
        ...
        <div class="sample-stack">
          ${loop.taskSamples.map((sample) => `
            <article class="sample-card">
              <div class="sample-main">
                <div class="badge-row">
                  <span class="badge deep">${escapeHtml(sample.id)}</span>
                  <span class="badge">${escapeHtml(sample.role)}</span>
                </div>
                <h3>${escapeHtml(sample.title)}</h3>
                <p>${escapeHtml(sample.openingCase)}</p>
                <p>${escapeHtml(sample.studentRead)}</p>
                ${sample.deepHref ? `<a class="deep-link" href="${escapeHtml(sample.deepHref)}">${escapeHtml(sample.deepLabel || "打开深样章")}</a>` : ""}
                ${sample.deepCandidate ? `<span class="badge deep">深样章候选</span>` : ""}
              </div>
              <div class="sample-side">
                <h4>学生要完成</h4>
                <ol>
                  ${sample.steps.map((step) => `<li>${escapeHtml(step)}</li>`).join("")}
                </ol>
                <div class="completion-box">
                  <strong>完成标准</strong>
                  <span>${escapeHtml(sample.completion)}</span>
                </div>
                <div class="completion-box">
                  <strong>教师带教</strong>
                  <span>${escapeHtml(sample.teacherUse)}</span>
                </div>
              </div>
            </article>
          `).join("")}
  ...
  function renderProject() {
    ...
    workspace.innerHTML = `
      <section class="section">
        <div class="badge-row">
          <span class="badge">${escapeHtml(project.id)}</span>
          <span class="badge ${project.id === "P2" ? "deep" : "warn"}">${escapeHtml(project.status)}</span>
        ...
        <div class="task-list">
          ${project.tasks.map((task) => `
            <article class="task-row">
              ...
              <div>
                ${task.deepSample && data.meta.deepSampleHref ? `<a class="deep-link" href="${escapeHtml(data.meta.deepSampleHref)}">打开深样章</a>` : task.deepCandidate ? `<span class="badge deep">深样章候选</span>` : `<span class="badge ${task.status.includes("样稿") ? "deep" : "warn"}">${escapeHtml(task.status.includes("样稿") ? "任务样稿" : task.status.includes("分支") ? "条件分支" : "结构占位")}</span>`}
              </div>
            </article>
          `).join("")}
        </div>
      </section>
      ${project.id === "P2" ? renderPhase2Loop() : ""}
      <section class="section">
        <h2>项目层判断</h2>
        <div class="quality-card">
          <p>${escapeHtml(project.id === "P2" ? "当前项目页已经进入阶段3闭环复制验证：P2-T1是测试采集主线，P2-T2是异常处理分支，P2-T3是分析输出和深样章候选。它仍不是专业定稿，也不是正式出版页面。" : "当前项目页不是完整教学内容页。它的任务是说明项目目标、任务链、能力群和开发状态。")}</p>
        </div>
      </section>
    `;
  }
  ...
  function renderGraph() {
    ...
    const p2LoopNodes = data.graph.detailNodes
      .filter((item) => ["P2-T1", "P2-T2", "P2-T3"].includes(normalizeGraphNode(item).task))
      .map(normalizeGraphNode);
    const visibleRelations = node.id && !node.id.startsWith("CG-") ? edgesForNode(node.id).slice(0, 8) : [];
    const relatedProject = data.projects.find((project) => project.id === node.project) || data.projects[1];
    workspace.innerHTML = `
      <section class="section">
        <h2>课程能力图谱</h2>
        ...
        <div class="graph-layout">
          <div>
            ...
            <h3 class="detail-heading">项目二闭环节点</h3>
            <p class="helper-text">本层来自V0.3.13项目二19个详细节点，用于观察P2-T1测试采集主线、P2-T2异常处理分支和P2-T3数据分析输出如何衔接。</p>
            <div class="detail-chain p4-loop-chain">
              ${p2LoopNodes.map((item) => `
                <button class="graph-node ${item.id === state.graphId ? "is-active" : ""}" data-graph="${item.id}" type="button">
                  <strong>${escapeHtml(item.id)} ${escapeHtml(item.label)}</strong>
  ...
  function renderTeacher() {
    workspace.innerHTML = `
      <section class="section">
        <h2>教师带教视图</h2>
        <p class="lead">教师视图说明整本教材如何组织课堂，不进入学生学习路径。当前重点验证项目二主线与异常分支的教学组织方式。</p>
      ...
      <section class="section">
        <h2>项目二教师组织要点</h2>
        <div class="quality-card">
          <p>教师需要先控制P2-T1的任务确认和数据交付质量，再根据测试异常决定是否进入P2-T2；数据可用后进入P2-T3讲评指标解释、异常证据链和分析报告表达。</p>
        </div>
      </section>
    `;
  }
    ...
    workspace.innerHTML = `
      ...
      <section class="section">
        ...
        <ol class="quality-list">
          ...
          <li>P2-T1、P2-T2、P2-T3教学模拟案例继续标注待专业复核。</li>
          <li>真实LOG、GPS轨迹、设备照片和软件截图不得默认发布。</li>
