...
export default async function PresentNodePage({ params }: { params: Promise<{ nodeId: string }> }) {
  const { nodeId } = await params;
  if (nodeId !== p4NodeExperience.nodeId) return <AuthGate role="teacher"><GenericNodeExperience nodeId={nodeId} mode="present" /></AuthGate>;
  return <AuthGate role="teacher"><P4NodeExperience mode="present" /></AuthGate>;
}
