window.P4T2_GRAPH_DRIVEN = {
  meta: {
    ...
    promise: "通过5个递进案例，把投诉线索、指标依据和结论表达连成一次可靠的优化验收。",
    simulationNote: "本任务使用教学化案例，指标目标服务于本课训练。",
    modes: {
      class: "课堂带学",
      self: "自学跟练"
    }
  ...
  learningEntry: {
    firstStep: "先做第1例：把4条投诉线索归类。",
    output: "完成5个递进案例，最后交一段可讲评的验收结论。",
    standard: "完成标准：能说明每条投诉为什么对应覆盖、移动性、体验或容量中的一种。",
    finalOutput: "最终要形成：一段包含判断、依据、边界和建议的验收结论，并至少提交3条自己的修正句。"
  },
  courseGraph: {
    courseChain: [
  ...
  cases: [
    {
      ...
      completionStandard: "能把投诉线索对应到需要验证的数据类型，并说明为什么网页变好不等于全部验收通过。",
      whyPractice: "这一步训练把学生听到的自然语言投诉转成后续要查看的数据类型。",
      repairHint: "如果归类不稳，先问自己：这句话是在说信号强弱、移动中断、业务体验，还是高峰容量？",
      revisionTask: "用一句话说明：为什么“网页好了”仍不能直接写成全部验收通过。",
      revisionPlaceholder: "例如：网页打开顺畅只能说明一种业务体验改善，还要补看视频、直播和晚高峰容量。",
    ...
    },
    {
      ...
      completionStandard: "能排出移动性验证流程，并说明为什么不能只用静止点覆盖数据做验收结论。",
      whyPractice: "这一步训练学生先识别投诉发生在移动路径上，再补看切换和保持数据。",
      repairHint: "如果流程顺序错误，回到场景关键词“从电梯口走向食堂”，先判断场景，再看数据。",
      revisionTask: "用一句话说明：为什么覆盖达标后仍要补看移动性数据。",
      revisionPlaceholder: "例如：静止点覆盖达标只能说明站着测信号可以，走动过程中是否切换失败还要看切换成功率和重建次数。",
    ...
    },
    {
      ...
      completionStandard: "能区分平均值达标和尾部体验未闭环，并把未达标指标写进验收边界。",
      whyPractice: "这一步训练学生不要只用好看的平均值下结论，而要补看较差体验和高峰容量。",
      repairHint: "如果标注不稳，先把每行数据分成两类：可支持通过，或必须写成边界。",
      revisionTask: "用一句话改写本例边界：平均速率好时，为什么仍不能说全部达标。",
      revisionPlaceholder: "例如：平均速率达标可说明多数体验改善，但95分位时延、PRB利用率和卡顿次数未达标，晚高峰尾部体验仍未闭环。",
    ...
    },
    {
      ...
      completionStandard: "能把材料分成通过依据、边界和背景，并据此写出部分达标的理由。",
      whyPractice: "这一步训练半独立整理依据链，避免把优化前数据或背景描述当成验收依据。",
      repairHint: "如果分类不稳，先判断这条材料能不能直接证明优化后达到本课目标。",
      revisionTask: "用一句话说明：哪条材料只能作背景，不能当作通过依据。",
      revisionPlaceholder: "例如：优化前直播上行速率只能说明原来问题严重，不能证明优化后已经达到目标。",
    ...
    },
    {
      ...
      completionStandard: "能写出包含判断、依据、边界和建议的验收结论，并避免过度判断。",
      whyPractice: "这一步训练把前面读到的数据转成岗位上可提交、可讲评的职业表达。",
      repairHint: "如果结论不合格，先检查四件事：有没有判断、有没有依据、有没有边界、有没有后续建议。",
      revisionTask: "把本例结论改写成一段完整职业表达，必须包含判断、依据、边界和建议。",
      revisionPlaceholder: "例如：本次招聘会扩容建议判断为部分达标。接入、页面打开和掉线达到本课目标，但简历上传95分位时延未闭环，建议补充上传日志和高峰复测。",
  ...
  glossary: [
    {
      term: "SS-RSRP",
      explain: "5G同步信号参考信号接收功率，可以粗略理解为终端接收到的信号强度。",
      howToRead: "本课看覆盖是否改善时，先看SS-RSRP覆盖率是否达到本课目标。",
      misconception: "SS-RSRP好不代表视频、上传或移动过程一定好。",
      relatedCases: ["宿舍区投诉复盘", "地下食堂切换失败"]
    },
    {
      term: "SS-SINR",
      explain: "反映信号质量和干扰情况，数值好通常说明信号更干净。",
      howToRead: "和SS-RSRP一起看，判断静止点覆盖和质量是否具备通过依据。",
      misconception: "信号质量达标不能替代切换成功率、时延和容量判断。",
      relatedCases: ["地下食堂切换失败"]
    },
    {
      term: "95分位时延",
      explain: "把很多次时延从低到高排列后，靠近较差体验一侧的代表值。",
      howToRead: "本课用它检查尾部体验，避免只看平均值。",
      misconception: "平均值达标时，95分位仍可能不达标。",
      relatedCases: ["实训楼尾部体验", "招聘会扩容验收"]
    },
    {
      term: "PRB利用率",
      explain: "无线资源块的使用比例，可以帮助判断小区资源是否紧张。",
      howToRead: "晚高峰PRB利用率偏高时，要把容量边界写进结论。",
      misconception: "速率提升不等于容量压力消失。",
      relatedCases: ["实训楼尾部体验"]
    },
    {
      term: "切换成功率",
      explain: "终端移动时从一个小区切到另一个小区的成功比例。",
      howToRead: "投诉发生在移动路径上时，必须补看切换成功率、重建和掉线日志。",
      misconception: "静止点覆盖好，移动中也可能切换失败。",
      relatedCases: ["地下食堂切换失败", "体育馆活动保障"]
    },
    {
      term: "RRC连接成功率",
      explain: "终端建立无线连接的成功比例，常用于观察接入能力。",
      howToRead: "招聘会等高并发场景中，接入成功率可作为容量接入依据之一。",
      misconception: "能接入不代表上传、页面打开和保持性都达标。",
      relatedCases: ["招聘会扩容验收"]
    },
    {
      term: "弱覆盖采样点",
      explain: "测试中信号强度低于要求的位置或采样点。",
      howToRead: "弱覆盖采样点减少，说明覆盖问题改善。",
      misconception: "弱覆盖减少只是覆盖维度的依据，不等于全部业务体验达标。",
      relatedCases: ["宿舍区投诉复盘"]
    },
    {
      term: "尾部体验",
      explain: "不是多数人的平均体验，而是较差一部分样本的体验。",
      howToRead: "看95分位、卡顿次数、上传较差时延等指标。",
      misconception: "平均速率高时，尾部体验仍可能不合格。",
      relatedCases: ["实训楼尾部体验", "招聘会扩容验收"]
    },
    {
      term: "通过依据",
      explain: "能直接支持某项验收目标已经达到的数据或材料。",
      howToRead: "只有优化后达到本课目标的数据，才适合作为通过依据。",
      misconception: "优化前数据和背景描述不能单独证明优化后达标。",
      relatedCases: ["体育馆活动保障"]
    },
    {
      term: "边界",
      explain: "说明本次结论不能说得太满的限制条件或未达标数据。",
      howToRead: "凡是未达到本课目标、样本不足或场景没覆盖的内容，都要写成边界。",
      misconception: "写边界不是否定优化效果，而是让验收结论更可靠。",
      relatedCases: ["全部案例"]
    }
  ],
  teacher: {
    ...
    aiSuggestions: [
      {
        ...
        title: "任务组织建议",
        suggestion: "先用投诉线索引出验证对象，再组织覆盖、移动性、体验容量和结论表达四类训练。",
        teacherAction: "教师确认本班是否采用五例完整流程，或删减为三例重点练习。",
        output: "两课时任务组织方案"
      },
      {
        id: "AI-P4T2-REVIEW-01",
        status: "teacher_adjusted",
        title: "讲评建议",
        suggestion: "优先讲评平均值与95分位、通过依据与边界、四段式验收结论。",
        teacherAction: "教师结合学生提交的修正句选择两类高频错误公开讲评。",
        output: "课堂讲评脚本"
      },
      {
        id: "AI-P4T2-CHECK-01",
        status: "generated_pending_confirm",
        title: "复核建议",
        suggestion: "案例阈值和结论口径仍需通信专业教师复核，当前只能作为教学模拟目标。",
        teacherAction: "教师标注可课堂使用口径和必须待专业复核口径。",
        output: "专业复核清单"
      }
    ],
    flow: [
      "第1课时0-8分钟：用投诉线索归类进入“识别验证场景”。",
      "第1课时8-25分钟：围绕覆盖和移动性建立验证路径。",
      "第1课时25-45分钟：用平均值与95分位对照修正第一次判断。",
      "第2课时45-63分钟：学生整理通过依据、边界和背景。",
      "第2课时63-82分钟：学生拼装并提交验收结论。",
      "第2课时82-90分钟：教师基于修正句和模拟学情讲评。"
    ],
    groupWork: [
      "场景组：负责从投诉文字中提取验证对象。",
    ...
    risks: [
      "本任务围绕项目四任务2展开，帮助学生把优化结果验证做成可讲评、可提交的学习产出。",
      "案例采用教学化场景，教师可结合班级基础调整讲评深度。",
      "课堂使用时应把重点放在投诉归类、指标判断、依据选择和职业表达。"
    ],
    schedule: [
      { time: "第1课时 0-8分钟", action: "投屏首屏学习入口，让学生说出本任务2课时内要交什么。", output: "学生能复述最终产出是验收结论和修正句。" },
      { time: "第1课时 8-25分钟", action: "完成入门样例和对照样例A，教师追问为什么覆盖不是全部。", output: "形成投诉归类、验证流程和前两条修正句。" },
      { time: "第1课时 25-45分钟", action: "完成对照样例B，板书平均值、95分位和容量边界。", output: "形成指标标注和一条边界表达。" },
      { time: "第2课时 45-63分钟", action: "完成半扶手练习，学生把材料分为通过依据、边界和背景。", output: "形成依据分类和一条背景材料说明。" },
      { time: "第2课时 63-82分钟", action: "完成独立实践，学生提交四段式验收结论。", output: "形成可讲评的完整验收结论。" },
      { time: "第2课时 82-90分钟", action: "用修正句、模拟学情和典型答案组织讲评，布置课后自学修订。", output: "明确本任务仍需补强的专业点和表达点。" }
    ],
    boardAdvice: [
      "板书主线：投诉线索 -> 验证对象 -> 指标依据 -> 结论边界。",
      "投屏建议：先投当前案例和学习活动，不要一开始打开完整图谱。",
      "讲评时保留一条错误结论，让学生指出缺少依据还是缺少边界。"
    ],
    questionScripts: [
      "这句话是在说覆盖、移动性、业务体验还是容量？你凭什么判断？",
      "如果只看SS-RSRP达标，哪个投诉仍然无法解释？",
      "平均速率达标时，为什么还要看95分位？",
      "这条材料是通过依据、边界，还是背景？它能直接证明什么？",
      "这段结论能不能交给项目经理？还缺哪一句？"
    ],
    exemplarAnswers: [
      { caseId: "demo-dorm", answer: "宿舍区可判断为部分达标。覆盖和网页体验改善，但直播上行95分位时延仍未达本课目标，需要补充晚高峰上行复测。" },
      { caseId: "contrast-canteen", answer: "地下食堂不能只凭覆盖验收。静止点覆盖达标，但电梯口切换成功率和重建次数未达本课目标，移动性未闭环。" },
      { caseId: "contrast-lab", answer: "实训楼平均速率可作为改善依据，但95分位时延、PRB利用率和卡顿次数仍是边界，不能写成全部达标。" },
      { caseId: "guided-gym", answer: "体育馆覆盖、扫码支付和平均上行速率可作通过依据，人群移动切换成功率未达目标，建议活动模拟复测。" },
      { caseId: "mission-jobfair", answer: "招聘会扩容建议判断为部分达标。接入、页面打开和掉线达到本课目标，简历上传95分位时延未闭环，需补充上传日志和高峰复测。" }
    ],
    modeGuide: {
      class: [
        "课堂模式下，本任务按2课时组织。第1课时重在看懂投诉和读数据，第2课时重在整理依据和写结论。",
        "教师每完成一例就暂停一次，要求学生提交一条修正句。",
        "教师不要直接公布答案，先让学生在小组内说出“为什么”。",
        "最后一例用于讲评学生产出，不再重新讲概念。"
      ],
      self: [
        "自学模式可作为两课时后的课后30分钟巩固任务，要求学生至少重做2个未掌握案例。",
        "学生提交前必须用术语字典检查不懂的指标。",
        "教师验收时重点看独立实践结论是否包含判断、依据、边界和建议。"
      ]
    },
    trialGuide: [
      "试看目标：验证学生是否能在2课时内完成5个递进案例，并写出至少3条有意义的修正句。",
      "教师观察重点：学生是否知道第一步做什么，是否能解释数据能支持什么和不能说明什么。",
      "课堂记录方式：每个小组保留一条错误修正前表达和一条修正后表达，用于第二课时末讲评。",
      "试看后反馈：教师记录卡点、耗时、专业疑点和建议删改的案例。"
    ],
    studentObservation: [
      "进入页面后，学生能否在1分钟内说出本任务最终要交什么。",
      "学生完成每例活动后，能否写出自己的修正句，而不是只读参考答案。",
      "学生是否能主动查术语字典解决SS-RSRP、95分位、PRB等术语。",
      "学生最终结论是否包含判断、依据、边界和建议四部分。"
    ],
    simulatedAnalytics: {
      note: "以下为班级学情示例，供教师组织讲评时参考。",
      completion: [
        { label: "投诉线索归类", rate: "88%", priority: "低" },
        { label: "验证流程排序", rate: "74%", priority: "中" },
        { label: "指标行标注", rate: "61%", priority: "高" },
        { label: "依据分类", rate: "66%", priority: "高" },
        { label: "结论拼装", rate: "58%", priority: "高" }
      ],
      misconceptions: [
        "把覆盖达标直接写成全部验收通过。",
        "把平均速率达标等同于尾部体验达标。",
      ...
      reviewPriority: [
        "优先讲评95分位和平均值的区别。",
        "第二优先讲评通过依据、边界和背景的分类。",
        "最后讲评职业表达中判断、依据、边界、建议四段结构。"
      ]
    },
    reviewChecklist: [
      "案例阈值是否适合作为本课目标？",
      "指标解释是否符合通信专业表述？",
      "教学模拟案例是否会误导学生认为是行业通用标准？",
      "是否需要替换或重绘原教材中的非通用仿真截图？",
      "正式发布前是否需要补充来源或专家审核记录？"
    ],
    professionalReviewRows: [
      { caseId: "demo-dorm", item: "直播上行95分位时延≤90ms", sourceType: "AI教学模拟/本课目标", risk: "需确认目标值是否适合职业院校课堂案例", reviewerAction: "通信教师复核阈值和结论边界" },
      { caseId: "contrast-canteen", item: "电梯口切换成功率≥98%、10次往返重建≤1次", sourceType: "AI教学模拟/本课目标", risk: "需确认移动性指标组合是否符合实训教学表达", reviewerAction: "复核切换、重建和覆盖关系" },
      { caseId: "contrast-lab", item: "下行平均速率、95分位时延、PRB利用率、卡顿次数", sourceType: "公开规范方向+AI教学模拟", risk: "3GPP只能支撑指标方向，不能直接证明本课阈值", reviewerAction: "复核平均值/尾部体验教学逻辑" },
      { caseId: "guided-gym", item: "扫码支付成功率、直播上行平均速率、人群移动切换成功率", sourceType: "AI教学模拟/场景化案例", risk: "活动保障场景是否符合校园网络案例需要专业确认", reviewerAction: "复核业务指标和活动保障边界" },
      { caseId: "mission-jobfair", item: "RRC连接成功率、页面打开平均时延、上传95分位时延、掉线次数", sourceType: "公开规范方向+AI教学模拟", risk: "上传尾部体验和接入能力的关系需复核", reviewerAction: "复核最终验收结论口径" }
    ]
  },
  sources: [
