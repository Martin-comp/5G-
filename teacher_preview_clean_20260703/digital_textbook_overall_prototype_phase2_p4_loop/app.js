(function () {
  ...
  function renderContextPanel() {
    ...
    contextPanel.innerHTML = `
      ...
      <div class="panel-stack">
        ...
        <div class="panel-item">
          <strong>下一步产物</strong>
          <span>samples/digital_textbook_overall_prototype_phase2_p4_loop/ 完成后进入浏览器QA和治理更新。</span>
        </div>
        <div class="panel-item">
          <strong>外部门禁</strong>
          ...
          ${loop.route.map((item, index) => `
            <article class="route-card">
              <div class="badge-row">
                ...
                <span class="badge">${escapeHtml(index === 0 ? "入口" : index === 1 ? "中枢" : "出口")}</span>
              </div>
          ...
          ${loop.taskSamples.map((sample) => `
            <article class="sample-card">
              <div class="sample-main">
                ...
                ${sample.id === "P4-T2" ? `<a class="deep-link" href="${escapeHtml(data.meta.deepSampleHref)}">打开P4-T2深样章</a>` : ""}
              </div>
        ...
        <div class="task-list">
          ${project.tasks.map((task) => `
            <article class="task-row">
              ...
              <div>
                ${task.deepSample ? `<a class="deep-link" href="${escapeHtml(data.meta.deepSampleHref)}">打开深样章</a>` : `<span class="badge ${task.status.includes("轻量") ? "deep" : "warn"}">${escapeHtml(task.status.includes("轻量") ? "轻量样稿" : "结构占位")}</span>`}
              </div>
            </article>
          `).join("")}
        </div>
      </section>
      ${project.id === "P4" ? renderPhase2Loop() : ""}
      <section class="section">
        <h2>项目层判断</h2>
        <div class="quality-card">
          <p>${escapeHtml(project.id === "P4" ? "当前项目页已经进入阶段2小范围闭环样本：P4-T1和P4-T3用于轻量教材样稿验证，P4-T2继续作为深样章质量锚点。它仍不是专业定稿，也不是正式出版页面。" : "当前项目页不是完整教学内容页。它的任务是说明项目目标、任务链、能力群和开发状态。")}</p>
        </div>
      </section>
    `;
  }
  ...
  function renderGraph() {
    ...
    const p4LoopNodes = data.graph.detailNodes
      .filter((item) => ["P4-T1", "P4-T2", "P4-T3"].includes(normalizeGraphNode(item).task))
      .map(normalizeGraphNode);
      ...
      <section class="section">
        ...
        <div class="graph-layout">
          <div>
            ...
            <h3 class="detail-heading">项目四闭环节点</h3>
            <p class="helper-text">本层来自V0.3.13项目四20个详细节点，用于观察P4-T1、P4-T2、P4-T3如何从实施证据进入验证，再进入报告表达。</p>
            <div class="detail-chain p4-loop-chain">
              ${p4LoopNodes.map((item) => `
                <button class="graph-node ${item.id === state.graphId ? "is-active" : ""}" data-graph="${item.id}" type="button">
                  <strong>${escapeHtml(item.id)} ${escapeHtml(item.label)}</strong>
