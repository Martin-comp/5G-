    ...
    $("#teacherView").innerHTML = `
      <section class="teacher-shell">
        <div class="section-head">
          ...
          <p>教师页用于组织两课时课堂节奏、投屏板书、观察学生修正产出和讲评学习单，不进入学生主学习路径。</p>
        </div>
        <div class="teacher-grid">
          ...
          <article class="teacher-card wide">
            ...
            <div class="review-table">
              <table>
                <thead>
                  <tr><th>案例</th><th>复核项</th><th>来源类型</th><th>风险</th><th>复核动作</th></tr>
                </thead>
                <tbody>
                  ${data.teacher.professionalReviewRows.map((row) => {
                    const caseItem = data.cases.find((item) => item.id === row.caseId);
                    return `
                      <tr>
                        <td>${esc(caseItem?.shortTitle || row.caseId)}</td>
                        <td>${esc(row.item)}</td>
                        <td>${esc(row.sourceType)}</td>
                        <td>${esc(row.risk)}</td>
                        <td>${esc(row.reviewerAction)}</td>
                      </tr>
                    `;
                  }).join("")}
                </tbody>
