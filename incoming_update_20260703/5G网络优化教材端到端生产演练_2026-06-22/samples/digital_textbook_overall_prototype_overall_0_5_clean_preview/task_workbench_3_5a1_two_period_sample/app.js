(function () {
  ...
  const storageKey = "p4t2_graph_driven_3_5a1_state";
  ...
  function nodeListText(ids, fallback) {
    return (ids || []).map(nodeDisplayName).join("、") || fallback;
  }
      ...
      <div class="student-shell">
        <section class="hero">
          ...
          <div class="hero-actions">
            ...
            <button id="openMap" type="button" class="secondary-button">本任务图谱</button>
            <button id="resetLearning" type="button" class="ghost-button">重新开始</button>
          </div>
        </section>
  ...
  function renderGraphContext(item) {
    ...
    return `
      <section class="graph-context">
        <div>
          <p class="eyebrow">当前任务图谱位置</p>
          <h4>${nodes.map((node) => `我正在练：${esc(node.label)}`).join(" / ")}</h4>
          <p>这些节点说明本例为什么这样练、练完后能做什么。点击图谱可查看学习前后关系和相关材料。</p>
        </div>
        <button type="button" class="secondary-button small-map" id="openMapInline">查看本任务节点</button>
      </section>
    `;
  }
    ...
    return `
      <div class="feedback ${result.complete ? "good" : "bad"}">
        ...
        <div class="repair-path">
          ...
          <button type="button" class="secondary-button" data-scroll-revision="${esc(item.id)}">写修正句</button>
        </div>
      </div>
    `;
  }

  function renderRevisionBox(item) {
    const store = currentActivityState(item);
    const text = store.revisionText || "";
    const ready = activityComplete(item);
    return `
      <section class="revision-box" aria-label="学生修正句">
        <div>
          <p class="eyebrow">二次修正</p>
          <h4>把反馈改成自己的话</h4>
          <p>${esc(item.revisionTask)}</p>
        </div>
        <label>
          <span>我的修正句</span>
          <textarea data-revision-input="${esc(item.id)}" rows="4" placeholder="${esc(item.revisionPlaceholder)}">${esc(text)}</textarea>
        </label>
        <p class="hint">${ready ? "活动已完成。请至少写一句自己的修正表达，教师讲评会优先看这里。" : "先完成学习活动，再根据反馈写修正句。"}</p>
      </section>
    `;
  }
  ...
  function renderLearningSheet(item) {
    ...
    const store = currentActivityState(item);
    const revision = (store.revisionText || "").trim();
    const reference = [
      item.conclusion.judgement,
      item.conclusion.evidence.join(""),
      item.conclusion.boundary,
      item.conclusion.next
    ].join("");
    return `
      <section class="learning-sheet" aria-label="结构化学习单">
        <div>
          <p class="eyebrow">本例学习单</p>
          <h4>${done && revision ? "已形成可讲评提交物" : "先完成活动，再写自己的修正句"}</h4>
          <p>学习单分为“我的填写”和“参考表达”。教师讲评优先看学生自己的修正句，不把参考答案当作学生产出。</p>
        </div>
        <div class="sheet-grid two-layer-sheet">
          <article class="student-output"><span>我的填写</span><p data-student-output="${esc(item.id)}">${esc(revision || "尚未填写。请在上方二次修正区写一句自己的判断、依据或边界。")}</p></article>
          <article><span>参考判断</span><p>${esc(item.conclusion.judgement)}</p></article>
          <article><span>参考依据</span><p>${item.conclusion.evidence.map((line) => esc(line)).join("<br>")}</p></article>
          <article><span>参考边界</span><p>${esc(item.conclusion.boundary)}</p></article>
          <article><span>参考建议</span><p>${esc(item.conclusion.next)}</p></article>
          <article class="reference-expression"><span>参考完整表达</span><p>${done ? esc(reference) : "完成活动后再显示参考完整表达。"}</p></article>
        </div>
      </section>
    `;
  }
  ...
  function renderGlossary() {
    ...
    return `
      <section class="side-card glossary-card">
        ...
        <p class="dictionary-note">词条库用于查询本任务中的关键指标和常见误解。</p>
      </section>
    `;
  }
  ...
  function renderSourceNote() {
    return `
      <section class="side-card subtle">
        <p class="eyebrow">案例说明</p>
        <p>本任务使用教学化案例，帮助你练习从投诉、指标和依据形成验收结论。</p>
      </section>
    `;
  }
    ...
    $("#teacherView").innerHTML = `
      <section class="teacher-shell">
        <div class="section-head">
          <p class="eyebrow">AI教学组织</p>
          <h2>${esc(data.teacher.title)}</h2>
          <p>AI预生成任务组织、课堂讲评和复核建议，教师审核确认后形成可执行教学安排。</p>
        </div>
        <div class="teacher-grid">
          <article class="teacher-card wide">
            <h3>AI建议与教师确认</h3>
            <div class="ai-suggestion-grid">
              ${data.teacher.aiSuggestions.map((item) => `
                <article class="ai-card">
                  <span>${esc(item.id)}</span>
                  <h4>${esc(item.title)}</h4>
                  <b>${esc(aiStatusLabel(item.status))}</b>
                  <p>${esc(item.suggestion)}</p>
                  <small><strong>教师动作：</strong>${esc(item.teacherAction)}</small>
                  <small><strong>形成产出：</strong>${esc(item.output)}</small>
                </article>
              `).join("")}
            </div>
          </article>
          ...
          <article class="teacher-card wide">
            <h3>课堂观察说明</h3>
            <ul>${data.teacher.trialGuide.map((line) => `<li>${esc(line)}</li>`).join("")}</ul>
          </article>
          <article class="teacher-card wide">
            <h3>学生观察记录点</h3>
            <ul>${data.teacher.studentObservation.map((line) => `<li>${esc(line)}</li>`).join("")}</ul>
          </article>
    ...
    $("#resourcesView").innerHTML = `
      <section class="resources-shell">
        <div class="section-head">
          ...
          <p>本页说明哪些内容可以转成表、图、动画、互动或小游戏，并对应到具体学习活动。</p>
        </div>
  ...
  function renderMap() {
    ...
    $("#mapBody").innerHTML = `
      ...
      <div class="task-graph">
        ...
        ${data.courseGraph.taskNodes.map((node, index) => `
      ...
      <aside class="node-detail">
        <p class="eyebrow">学习关系</p>
        <h3>我正在练：${esc(selected.label)}</h3>
        <p>${esc(selected.summary || selected.desc)}</p>
        ${selected.goal ? `
          <dl>
            <dt>练什么</dt><dd>${esc(selected.goal)}</dd>
            <dt>先会什么</dt><dd>${esc(nodeListText(selected.prerequisites, "无明确前置"))}</dd>
            <dt>接着练什么</dt><dd>${esc(nodeListText(selected.next, "本任务到此收束"))}</dd>
        ...
        ` : `
          <p class="node-note">这是全课程主链节点，用来说明本任务在整门课程中的位置。</p>
        `}
      </aside>
    `;
  }
  ...
  function renderRelatedTask(node, caseItem) {
    if (!caseItem) {
      return `<p class="node-note">这个节点属于课程主链。请选择项目四任务节点查看对应学习活动。</p>`;
    }
    return `
  ...
  function bindStudent() {
    ...
    $$("[data-revision-input]").forEach((input) => {
      input.oninput = () => {
        const item = data.cases.find((caseItem) => caseItem.id === input.dataset.revisionInput) || currentCase();
        const store = currentActivityState(item);
        store.revisionText = input.value;
        save();
        const output = document.querySelector(`[data-student-output="${CSS.escape(item.id)}"]`);
        if (output) {
          output.textContent = input.value.trim() || "尚未填写。请在上方二次修正区写一句自己的判断、依据或边界。";
        }
      };
    });
    ...
    $("[data-scroll-revision]")?.addEventListener("click", () => {
      scrollToElement(".revision-box");
      window.requestAnimationFrame(() => {
        document.querySelector("[data-revision-input]")?.focus();
      });
    });
  ...
  function resetLearningState() {
    ...
    state.mode = "class";
    state.caseIndex = 0;
    state.selectedGraphNodeId = "P4T2-N01";
    state.glossaryQuery = "";
    state.activities = {};
    save();
    render();
