...
async function main() {
  ...
  await check("页面标题包含数字教材整体原型", async () => (await page.locator("body").innerText()).includes("数字教材整体原型"));
  await check("课程首页显示6个项目和18个任务", async () => {
    const text = await page.locator(".workspace").innerText();
    return text.includes("6") && text.includes("18") && text.includes("整书项目链");
  });
  await check("项目视图可进入P4并显示深样章入口", async () => {
    await page.locator('[data-view="project"]').click();
    const text = await page.locator(".workspace").innerText();
    const href = await page.locator(".deep-link").first().getAttribute("href");
    return text.includes("P4-T2") && text.includes("深样章已完成") && href.includes("task_workbench_3_5a1_two_period_sample");
  });
  await check("课程能力图谱节点可点击并显示关联任务", async () => {
    ...
    await page.locator('[data-graph="P4T2-N07"]').click();
    const text = await page.locator(".workspace").innerText();
  ...
  await check("教师视图显示外部门禁", async () => {
    ...
    const text = await page.locator(".workspace").innerText();
    return text.includes("外部门禁") && text.includes("通信专业复核");
  });
  await check("资源中心显示审核状态", async () => {
    await page.locator('[data-view="resources"]').click();
    const text = await page.locator(".workspace").innerText();
    return text.includes("审核") && text.includes("不可默认发布");
  });
